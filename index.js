/* INFO:
    ...
    문제가 있을경우 내리도록 하겠습니다!!
*/
const cver = '1.9'; // cver변수의 값 선언(현재 버전)
...
function winalr(t, b) {
    if (Notification.permission === 'granted') {
        let n = new Notification(t, { body: b, icon: 'https://cdn.discordapp.com/avatars/652090099940720642/831de832f07f47dcc24f0b1981f8d439.png?size=512' });
        setTimeout(function () {
            n.close();
        }, 10000);
    }
...
setInterval(function () { // 미소지진정보갱신, 여진정보갱신, 페이즈 상태 체크 1초마다 작동
    geteqk('https://www.weather.go.kr/weather/earthquake_volcano/ajaxEqkMicroPopup.jsp', function (d) {// 미소지진 갱신
        if (d && d.indexOf('<p class=\"p_hypen\">') > 0 && d.indexOf('</p>') > 0 && eqkdata[0] && eqkdata[0] != d) { // d 매개변수에 값이 있고 eqkdata[0] 변수에 값이 있고 eqkdata[0] 변수의 값과 d 매개변수의 값이 다른경우
    ...
    geteqk('https://www.weather.go.kr/weather/earthquake_volcano/ajaxEqkNoticePopup.jsp', function (d) {// 여진정보 갱신
        if (d && d.indexOf('<p class=\"p_hypen\">') > 0 && d.indexOf('</p>') > 0 && eqkdata[1] && eqkdata[1] != d) { // d 매개변수에 값이 있고 eqkdata[1] 변수에 값이 있고 eqkdata[1] 변수의 값과 d 매개변수의 값이 다른경우
